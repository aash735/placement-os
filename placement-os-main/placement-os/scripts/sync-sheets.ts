...
function main() {
  ...
  const weeklyPlan: { week: number; focus: string; hours: string; days: string[] }[] = [];

  weeklyRows.forEach((r, idx) => {
    const rowNum = idx + 2;
    const rawWeek = String(r.week || "").trim();
    const rawFocus = String(r.focus || "").trim();
    const rawHours = String(r.hours || "").trim();
    const rawDays = String(r.days || "").trim();

    // Skip completely empty rows
    if (!rawWeek && !rawFocus && !rawHours && !rawDays) {
      return;
    }

    const weekNum = Number(rawWeek);
    const parsedWeek = isNaN(weekNum) ? 0 : weekNum;

    if (!rawWeek || isNaN(weekNum)) {
      const msg = `Invalid or missing week number: '${rawWeek}' in Weekly Review plan.`;
      console.warn(`⚠️  [Weekly Plan Row ${rowNum}] ${msg}`);
      validationIssues.push({ row: rowNum, field: "week", message: msg });
    }

    if (!rawFocus) {
      const msg = `Missing focus description in Weekly Review plan for week ${parsedWeek}.`;
      console.warn(`⚠️  [Weekly Plan Row ${rowNum}] ${msg}`);
      validationIssues.push({ row: rowNum, field: "focus", message: msg });
    }

    if (parsedWeek > 0) {
      if (seenWeeks.has(parsedWeek)) {
        const msg = `Duplicate week number '${parsedWeek}' detected in Weekly Review plan.`;
        console.warn(`⚠️  [Weekly Plan Row ${rowNum}] ${msg}`);
        validationIssues.push({ row: rowNum, field: "week", message: msg });
      } else {
        seenWeeks.add(parsedWeek);
      }
    }
    ...
    weeklyPlan.push({
      ...
      focus: rawFocus,
      hours: rawHours,
      days: rawDays.split("|").map((d) => d.trim()).filter(Boolean),
    });
  });
